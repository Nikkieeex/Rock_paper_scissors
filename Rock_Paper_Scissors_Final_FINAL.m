...
function Rock_Paper_Scissors_V2()
    ...
    % GUI aanmaken
    f = figure('Name', 'RPS Automaton', 'Position', [100 100 1200 750], 'Color', [1,0.9,0.95]);
    movegui(f, 'center');
...
btn_colors = {
    ...
    uicontrol('Style','pushbutton','String','Quit','Units','normalized','Position',[btn_x y0-6*(btn_h+spacing) btn_w btn_h],...
        'BackgroundColor',[1 1 1],'FontSize',16,'Callback',@(src,ev) close(f));

    % Snelheidsschuifregelaar
    uicontrol('Style','text','String','fast -> slow','Units','normalized','Position',[btn_x, y0-7*(btn_h+spacing), btn_w, 0.03], ...
